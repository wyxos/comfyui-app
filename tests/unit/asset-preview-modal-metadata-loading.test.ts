...
describe('AssetPreviewModal image metadata loading', () => {
  ...
  it('loads metadata for model endpoint images whose id is only present in the Civitai CDN URL', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = new URL(String(input), 'http://127.0.0.1')
      if (url.pathname === '/api/civitai/images' && url.searchParams.get('imageId') === '134254134') {
        return new Response(
          JSON.stringify({
            items: [{
              id: 134254134,
              url: 'https://image.civitai.com/xG1nkqKTMzGDvpLrqFT7WA/00bed0b0-f493-4e56-aa31-1079f3dae8b2/original=true/00bed0b0-f493-4e56-aa31-1079f3dae8b2.jpeg',
              type: 'image',
              nsfw: false,
              meta: {
                prompt: 'refraction glasses, sunset',
                seed: 789448265,
              },
            }],
            metadata: {},
    ...
    const imageDetailRequest = fetchMock.mock.calls
      .map((call) => new URL(String(call[0]), 'http://127.0.0.1'))
      .find((url) => url.searchParams.get('imageId') === '134254134')
    expect(imageDetailRequest?.pathname).toBe('/api/civitai/images')
    expect(wrapper.text()).toContain('Image ID')
    expect(wrapper.text()).toContain('134254134')
    expect(wrapper.text()).toContain('refraction glasses, sunset')
    expect(wrapper.text()).not.toContain('No prompt metadata found for this image.')
  })

  it('lets the user retry metadata loading after an empty image details response', async () => {
    let imageDetailRequests = 0
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = new URL(String(input), 'http://127.0.0.1')
      if (url.pathname === '/api/civitai/images' && url.searchParams.get('imageId') === '901') {
        imageDetailRequests += 1
        if (imageDetailRequests <= 2) {
          return new Response(JSON.stringify({ items: [], metadata: {} }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          })
        ...
        return new Response(
          JSON.stringify({
            items: [{
              id: 901,
              url: 'https://example.test/retry-meta.jpg',
              type: 'image',
              nsfw: false,
              meta: {
                prompt: 'loaded after retry',
                seed: 901902,
              },
            }],
            metadata: {},
    ...
    const wrapper = mount(AssetPreviewModal, {
      ...
      props: {
        ...
        model: {
          id: 101,
          name: 'Metadata retry model',
          type: 'Checkpoint',
          modelVersions: [
            {
              id: 201,
              name: 'v1',
              images: [{ id: 901, url: 'https://example.test/retry-meta.jpg', type: 'image', nsfw: false }],
            },
          ],
        },
      },
    ...
    await wrapper.get('[data-test="asset-preview-metadata-retry"]').trigger('click')
    await flushPromises()
    await flushPromises()

    expect(imageDetailRequests).toBe(3)
