...
describe('AssetPreviewModal Atlas reactions', () => {
  ...
  it('keeps an already loaded feed copy active after reacting to the original media', async () => {
    const feedImages = [
      { id: 800, url: 'https://example.test/version-original.jpg', type: 'image', nsfw: false },
    ]
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const url = new URL(String(input), 'http://127.0.0.1')
      if (url.pathname === '/api/settings/app') {
    ...
    expect(
      wrapper
        .get('[data-test="asset-preview-feed-atlas-reactions"]')
        .get('button[aria-label="Like in Atlas"]')
        .attributes('aria-pressed'),
    ).toBe('true')
  })

  it('shows Atlas file actions for known files and clears them after delete', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.pathname === '/api/atlas/files/88' && init?.method === 'DELETE') {
        return new Response(JSON.stringify({
          ok: true,
          configured: true,
          deleted: true,
    ...
    expect(widget.get('a[aria-label="Open file in Atlas"]').attributes('href')).toBe('https://atlas.test/browse/file/88')
    await widget.get('button[aria-label="Delete downloaded file from Atlas"]').trigger('click')
    await flushPromises()

    const deleteCall = fetchMock.mock.calls.find((call) => String(call[0]) === '/api/atlas/files/88')
    expect(deleteCall?.[1]).toMatchObject({ method: 'DELETE' })
    expect(requestBody(deleteCall)).toMatchObject({
      also_from_disk: true,
      also_delete_record: true,
    })
    expect(wrapper.find('a[aria-label="Open file in Atlas"]').exists()).toBe(false)
    expect(wrapper.find('button[aria-label="Delete downloaded file from Atlas"]').exists()).toBe(false)
  })
