...
describe('LibraryView filters', () => {
  ...
  it('paginates local models with NSFW items hidden until the toggle is on', async () => {
    ...
    await wrapper.get('[aria-label="Next library page"]').trigger('click')
    await vi.waitFor(() => expect(wrapper.text()).toContain('Library model 41'))
    ...
    await vi.waitFor(() => expect(wrapper.text()).toContain('1-40 of 46'))
    await wrapper.get('[aria-label="Next library page"]').trigger('click')
    await vi.waitFor(() => expect(wrapper.text()).toContain('Library model 46'))
